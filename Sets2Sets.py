...
def generate_dictionary_BA(path, files, attributes_list):
    ...
    for filename in files:
        ...
        with open(path + filename, 'r') as csvfile:
            ...
            for row in reader:
                if count == 0: # ignore the headers of the csv file
                    count += 1
                    continue
                for att in range(len(attributes_list)):
                    key = attributes_list[att]
                    if row[2+att] not in dictionary_table[key]:
                        dictionary_table[key][row[2+att]] = counter_table[key]
    ...
    print('# dimensions of final vector (total number of unique values for the attributes | number of unique ones in future that are not in history): ' + str(total) + ' | ' + str(count - 1))
...
def read_claim2vector_embedding_file_no_vector(path, files):
    ...
    attributes_list = ['MATERIAL_NUMBER', 'TOP_CAT_NUMBER', 'SUB_CAT_NUMBER']
        ...
        with open(path + filename, 'r') as csvfile:
            ...
            for row in reader:
                ...
                for att in range(len(attributes_list)):
                    ...
                    # reindex the feature based on the position in attributes_list 
                    # so that all features get a unique number
                    ...
                    # note that some items might have the same categorical values and duplicates are ignored    
...
def main(argv):
    ...
    # print('code_freq_at_first_claim', code_freq_at_first_claim)
